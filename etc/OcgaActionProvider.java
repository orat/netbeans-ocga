...
//TODO
// was macht dieser ActionProvider? 
// Wie kann ich ocga.bat erzeugen?
// geht das dann nur unter windows?
// code stammt aus enso
@NbBundle.Messages({
    "CTL_OcgaWhere=OCGA Executable Location",
    "CTL_OcgaExecutable=ocga/ocga.bat",
    "# {0} - executable file",
...
public final class OcgaActionProvider implements ActionProvider {
    ...
    public void invokeAction(String action, Lookup lkp) throws IllegalArgumentException {
        ...
        var prefs = NbPreferences.forModule(OcgaActionProvider.class);
        var exeKey = "ocga.executable";

        var exe = prefs.get(exeKey, "");
        var nd = new NotifyDescriptor.InputLine(Bundle.CTL_OcgaExecutable(), Bundle.CTL_OcgaWhere());
        nd.setInputText(exe);
    ...
    public boolean isActionEnabled(String action, Lookup lkp) throws IllegalArgumentException {
        return switch (action) {
            case COMMAND_RUN_SINGLE, COMMAND_DEBUG_SINGLE, COMMAND_RUN, COMMAND_DEBUG ->
                lkp.lookup(ocgaDataObject.class) != null;
            default -> false;
        };
    }
